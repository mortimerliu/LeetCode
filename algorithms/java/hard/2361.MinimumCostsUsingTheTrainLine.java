/*
 * @lc app=leetcode id=2361 lang=java
 *
 ...
 * stop i using the regular route, and express[i] describes the cost it takes
 * to go from stop i - 1 to stop i using the express route.
 *
...
class Solution {
    public long[] minimumCosts(int[] regular, int[] express, int expressCost) {
        ...
        long minReg = 0, minExp = expressCost;
        long[] minCost = new long[n];
        for (int i = 0; i < n; i++) {
            long curMinReg = Math.min(minReg, minExp) + regular[i];
            long curMinExp = Math.min(minReg + expressCost, minExp) + express[i];
            minReg = curMinReg;
            minExp = curMinExp;
            minCost[i] = Math.min(minReg, minExp);
        }
        return minCost;
    }
}

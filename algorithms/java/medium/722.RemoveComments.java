 ...
 * to right.) To be clear, the string "/*\/" does not yet end the block comment,
    ...
    public List<String> removeComments(String[] source) {
        ...
        for (String line : source) {
            for (int i = 0; i < line.length(); i++) {
                if (!inBlock && line.charAt(i) == '/' && i < line.length() - 1 && line.charAt(i + 1) == '*') {
                    i++;
                    inBlock = true;
                } else if (!inBlock && line.charAt(i) == '/' && i < line.length() - 1 && line.charAt(i + 1) == '/') {
                    i = line.length();
                } else if (inBlock && line.charAt(i) == '*' && i < line.length() - 1 && line.charAt(i + 1) == '/') {
                    i++;
                    inBlock = false;
